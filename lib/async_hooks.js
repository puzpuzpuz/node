...
const asyncContextHook = createHook({
  init(asyncId, type, triggerAsyncId, resource) {
    ...
    for (let i = 0; i < localStorageRefs.length; ++i) {
      // TODO clean up empty refs
      const localStorage = localStorageRefs[i].get();
      if (localStorage) {
        localStorage._propagate(resource, currentResource);
      }
    }
    // Self-throttle
    if (localStorageRefs.length === 0) {
...
class AsyncLocalStorage {
  constructor() {
    ...
    localStorageRefs.push(new WeakReference(this));
  ...
  exitSyncAndReturn(callback, ...args) {
    const resource = executionAsyncResource();
    const store = resource[this.kResourceStore];
    resource[this.kResourceStore] = undefined;
    try {
      return callback(...args);
    } finally {
