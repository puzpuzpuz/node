...
let activeAsyncLocalStorageCount = 0;
const kLocalStorageList = Symbol('kLocalStorageList');
const asyncContextHook = createHook({
  init(asyncId, type, triggerAsyncId, resource) {
    const currentResource = executionAsyncResource();
    // Value of currentResource is always a non null object
    const localStorageList = currentResource[kLocalStorageList];
    if (!localStorageList || localStorageList.length === 0) {
      return;
    }
    for (let i = 0; i < localStorageList.length; ++i) {
      localStorageList[i]._propagate(resource, currentResource);
    }
  }
});
...
class AsyncLocalStorage {
  ...
  enable() {
    if (!this.enabled) {
      this.enabled = true;
      ++activeAsyncLocalStorageCount;
      asyncContextHook.enable();
      const resource = executionAsyncResource();
      let localStorageList = resource[kLocalStorageList];
      if (!localStorageList) {
        localStorageList = resource[kLocalStorageList] = [];
      }
      localStorageList.push(this);
    }
  }
  ...
  disable() {
    if (this.enabled) {
      this.enabled = false;
      --activeAsyncLocalStorageCount;
      const localStorageList = executionAsyncResource()[kLocalStorageList];
      if (localStorageList) {
        localStorageList.splice(localStorageList.indexOf(this), 1);
      }
      if (activeAsyncLocalStorageCount === 0) {
        asyncContextHook.disable();
      }
    }
  }
  ...
  _propagate(resource, triggerResource) {
    ...
    if (this.enabled) {
      ...
      let list = resource[kLocalStorageList];
      if (!list) {
        list = resource[kLocalStorageList] = [];
      }
      list.push(this);
    }
  }
