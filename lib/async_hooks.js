...
const asyncContextHook = createHook({
  init(asyncId, type, triggerAsyncId, resource) {
    ...
    contexts.forEach((context) => {
      context._propagate(resource, getResource(triggerAsyncId));
    });
  },
  destroy(asyncId) {
...
class AsyncContext {
  ...
  _propagate(resource, triggerResource) {
    const store = this._resourceStore.get(triggerResource);
    if (store) {
  ...
  /**
   * Get value from the current store, i.e. the unique map associated with current
   * async resource and (maybe) inherited from the parent async resource.
   * Will return null if no map is currently associated
   * or undefined if no value is present.
   */
  get(key) {
    const resource = getCurrentResource();
    if (resource) {
      const store = this._resourceStore.get(resource);
      if (store) {
  ...
  /**
   * Set value in the current store, i.e. the unique map associated with current
   * async resource and (maybe) inherited from the parent async resource.
   * This call has copy-on-write semantics, i.e. the previously existing store
   * (map) is copied.
   */
  set(key, value) {
    const resource = getCurrentResource();
    if (resource) {
      const oldStore = this._resourceStore.get(resource);
      if (!oldStore) {
        // TODO consider throwing an error in this case (the call was made outside of .run)
      }
      const newStore = new Map(oldStore);
      newStore.set(key, value);
      this._resourceStore.set(resource, newStore);
    }
  }

  /**
   * Calling get()/set() in the function will use a map internally.
   * If called within the callback chain of another run() call a new map will
   * replace the previous one.
   */
  run(fn) {
    // TODO consider introducing separate .enable()/.disable() methods
